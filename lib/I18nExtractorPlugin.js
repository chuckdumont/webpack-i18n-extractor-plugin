    ...
    tap(compiler, "compilation", (compilation, params) => {
      ...
      tap(compilation.mainTemplate, "startup", function(source, chunk) {
        ...
        if (options.promisePolyfill) {
          ...
          const promiseMod = promisePath && compilation.modules.find(module => module.request && module.request.includes(promisePath));
      ...
      tap(params.normalModuleFactory, "parser", (parser) => {
        tap(parser, "call require", expr => {
          if (expr.arguments.length !== 1) return;
          const param = parser.evaluateExpression(expr.arguments[0]);
          if (param.isConditional()) return;
      ...
      tap(compilation.mainTemplate, "hash", (hash) => {
        ...
        hash.update("7");   // Increment this whenever the render code above changes
      });
    });
  }
};
